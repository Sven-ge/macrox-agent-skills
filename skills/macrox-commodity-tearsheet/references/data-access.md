...
| 优先级 | 条件 | 做法 |
| --- | --- | --- |
| A | 会话里已有 MacroX 工具（如 `mkt_list_overview`、`struct_get_basis`） | **直调 MCP**，不要走脚本 |
| B | 无 MCP，但本机有 `macrox-futures-mcp` 且 Token 已写入 | 用该目录的 `call-node.js`（优先）或 `call.py` 调同名工具 |

## 禁止

- 宿主自带搜索 / 网页浏览 / 新闻插件（含 Kimi、ChatGPT、通义等内置检索）
- 其他 MCP、爬虫、行情网站、付费资讯 API
- 用模型记忆编造价格、涨跌幅、基差、仓单、席位数字
- 为凑「调用次数」或填满六维而改用上述来源

工具失败或无数据：该维标 **「数据不足 / 未取到」**，写入 brief。**不得**换源顶替。

## 探测 A

看当前会话是否已暴露 MacroX 工具名（`mkt_*` / `struct_*` / `news_*` / `seat_*` / `ref_*` / `factor_*` / `anl_*`）。有则走 A，跳过 B。
...
```bash
node -e "require('$SCRIPTS/call-node.js').call('mkt_list_overview',{symbols:'CU',limit:1}).then(r=>console.log(JSON.stringify(r))).catch(e=>{console.error(e);process.exit(1)})"
```

```bash
python3 -c "import json,sys; sys.path.insert(0,'$SCRIPTS'); from call import call; print(json.dumps(call('mkt_list_overview',{'symbols':'CU','limit':1}),ensure_ascii=False))"
```
...
**立刻停止**本工作流：不要写 brief、不要出 HTML、不要改用其他源。只回复：

```text
无法使用 MacroX 数据，已停止出稿。

本 Skill 禁止用搜索引擎或其他插件顶替行情/结构/席位数据。
